...
\ifthenelse{\boolean{formeln}}{
    \begin{equation}
        E_j = \frac{\hbar^2 }{2I}j(j + 1) = hc_0B_{\upsilon }j(j+1), \quad j = 0,1,2,... \quad \text{\cite[S.~293]{radiativeHeatTransfer}}
    \end{equation}

    $E = hf = hc_0\eta \Rightarrow \eta = \frac{E}{hc_0}$

    \begin{equation}
        E_{\upsilon} = hv_e(\upsilon + \frac{1}{2}), \quad \upsilon = 0,1,2,...
    \end{equation}

    \begin{equation}
        E_{\upsilon j} = hv_e(\upsilon + \frac{1}{2}) + B_{\upsilon }j(j+1), \quad \upsilon = 0,1,2,...
    \end{equation}
}{}
...
relativ zueinander bewegen, nichtlinear zunimmt. Wenn man daher eine komplexere Federkonstante in die Analyse einbezieht, führt dies zu
...
$\Delta \upsilon = \pm 2$, $\Delta \upsilon = \pm 3$, $\ldots$ als Obertöne bezeichnet werden \cite[S.~295]{radiativeHeatTransfer}.
...
wobei $\eta_0$ die Bandmitte bezeichnet. Die detaillierte Herleitung dieser Formeln 
findet sich in \autoref{sec:herleitung_3_zweige}. Bei der Bandmitte 
$\eta_0$ selbst tritt keine Spektrallinie auf.
