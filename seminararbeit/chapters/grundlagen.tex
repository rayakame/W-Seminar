...
Konzepte komplementär verwendet werden. \cite[S.~20--22]{radiativeHeatTransfer}
...
in Luft ($n \approx 1.0002$) stattfinden, wird im Folgenden die Näherung $n = 1 \Rightarrow c = c_0$.
...
und die infrarote Strahlung (IR, $\lambda > \SI{780}{\nano\meter}$) von Bedeutung sind. \cite[S.~22--24]{radiativeHeatTransfer} 
Elektromagnetische Wellen werden durch folgende Eigenschaften charakterisiert:
\begin{table}[H]
    \centering
    \small
    \begin{tabularx}{1\textwidth}{l X}
        ...
        \bottomrule
    \end{tabularx}
    \caption{Charakteristische Eigenschaften elektromagnetischer Wellen. \cite[S.~23]{radiativeHeatTransfer}}
...
\ifthenelse{\boolean{formeln}}{
  \begin{equation}
    ...
    \quad \text{\cite[S.~23]{radiativeHeatTransfer}}
  \end{equation}
}{}
...
Der von einer Oberfläche abgegebene Strahlungswärmestrom wird als \textit{spezifische Ausstrahlung} $E$ bezeichnet 
und quantifiziert die Intensität der Emission. \cite[S.~25--26]{radiativeHeatTransfer}

Dabei wird zwischen der \textit{gesamten spezifischen Ausstrahlung} $E$ und der \textit{spektralen spezifischen Ausstrahlung}
$E_{\nu}$ unterschieden. \cite[S.~26]{radiativeHeatTransfer}
\ifthenelse{\boolean{formeln}}{
  \begin{align*}
...
Dieses Gesetz wurde 1900 von Max Planck hergeleitet und ist heute als \textit{Plancksches Strahlungsgesetz} bekannt. \cite{plancknormalspektrum}
Für eine schwarze Oberfläche ergibt sich die spektrale spezifische Ausstrahlung \cite[S.~26]{radiativeHeatTransfer} zu:
\ifthenelse{\boolean{formeln}}{
  \begin{equation}
...
\ifthenelse{\boolean{formeln}}{
  \begin{equation}
    E = \int_{0}^{\infty}E_{\nu} \, d\nu \quad \text{\cite{stefanBoltzmannLaw}}
  \end{equation}
}{}
...
Die Erde bezieht nahezu ihre gesamte Energie von der Sonne. Um ein thermisches Gleichgewicht aufrechtzuerhalten, 
muss die abgestrahlte Leistung der empfangenen Leistung entsprechen \cite[S.~9]{marshall2007atmosphere}:
\ifthenelse{\boolean{formeln}}{
  \begin{equation}
...
Im globalen Mittel wird ein Anteil von $\alpha_p \simeq \SI{30}{\percent}$ der eingehenden Strahlung reflektiert. Diese Größe wird als \textit{planetare Albedo} bezeichnet. \cite[S.~11]{marshall2007atmosphere}
\ifthenelse{\boolean{abbildungen}}{
  \begin{figure}[H]
      \centering
...
wie ein idealer schwarzer Körper mit gleichmäßiger Temperatur $T_e$ verhält (bekannt als \textit{effektive planetare Temperatur}) und dass das 
\textit{Stefan-Boltzmann-Gesetz} wie in \autoref{sec:stefan_boltzmann} anwendbar ist. \cite[S.~11]{marshall2007atmosphere}
...
Die Erde strahlt über ihre gesamte Oberfläche $A = 4\pi r^2$ ab. \cite[S.~11]{marshall2007atmosphere} Umgestellt nach der Strahlungsleistung ergibt sich:
\ifthenelse{\boolean{formeln}}{
  \begin{equation}
...
Mit den Werten $S_0 = \SI{1361}{\watt\per\meter\squared}$, $\alpha_p = \SI{30}{\percent}$ und $\sigma = \SI{5.670e-8}{\watt\per\meter\squared\per\kelvin\tothe{4}}$ ergibt sich:
\ifthenelse{\boolean{formeln}}{
  \begin{align}
      T_e &= \left(\frac{\SI{1361}{\watt\per\meter\squared} \cdot (1-\SI{0.3}{})}{4 \cdot \SI{5.670e-8}{\watt\per\meter\squared\per\kelvin\tothe{4}}}\right)^{1/4} \nonumber\\
      &= \SI{254.6}{\kelvin}
...
Die Differenz von etwa \SI{33}{\kelvin} wird durch den natürlichen Treibhauseffekt der Atmosphäre verursacht. Lacis et al. schlussfolgern: \enquote{In round numbers, water vapor accounts for
...
\ifthenelse{\boolean{abbildungen}}{
  \begin{figure}[H]
      ...
      \caption{Vergleich der spektralen spezifischen Ausstrahlung von Sonne und Erde nach dem Planckschen Strahlungsgesetz mit den Absorptionsbanden von CO\textsubscript{2}. Grafik erstellt mit \autocite{python}, Code ist im Anhang \autoref{lst:planck} dokumentiert.}
